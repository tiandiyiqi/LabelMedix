...
- `label_width`: 标签宽度（单位：mm），项目级配置
- `label_height`: 标签高度（单位：mm），项目级配置
- `label_category`: 标签分类，如 "阶梯标"、"缠绕标"等，项目级配置
- `is_wrapped`: 是否为缠绕标（布尔值），项目级配置
- `created_at`: 创建时间
- `updated_at`: 更新时间
...
CREATE TABLE Projects (
    ...
    label_width DECIMAL(5,2) DEFAULT 100.00 COMMENT '标签宽度（单位：mm），项目级配置',
    label_height DECIMAL(5,2) DEFAULT 60.00 COMMENT '标签高度（单位：mm），项目级配置',
    label_category VARCHAR(50) DEFAULT '阶梯标' COMMENT '标签分类，如"阶梯标"、"缠绕标"等，项目级配置',
    is_wrapped BOOLEAN DEFAULT FALSE COMMENT '是否为缠绕标，项目级配置',
...
| 字段名         | 类型           | 默认值     | 说明                                                                 |
| -------------- | -------------- | ---------- | -------------------------------------------------------------------- |
| label_width    | DECIMAL(5,2)   | 100.00     | 标签宽度，单位为毫米（mm），项目级配置                               |
| label_height   | DECIMAL(5,2)   | 60.00      | 标签高度，单位为毫米（mm），项目级配置                               |
| label_category | VARCHAR(50)    | '阶梯标'   | 标签分类，如"阶梯标"、"缠绕标"等，项目级配置                        |
| is_wrapped     | BOOLEAN        | FALSE      | 是否为缠绕标，项目级配置                                             |

**配置优先级说明**：

- **项目级配置**：适用于整个项目的默认标签设置
- **国别级配置**：适用于特定国别的标签设置，优先级高于项目级配置
- **配置继承**：如果国别级配置未设置，则使用项目级配置作为默认值
